...
# F: CCPs -> values
# INPUT: P = CCPs, P is an x_m by 4 matrix with p0(x,s) and p1(x,s) for all x = 0,1,...x_m and s = s_1, s_2, where
...
#                               the engine of brand s at mileage x, given P
#             Note: This is an x_m by 2 matrix, where
...
Foperator <- function(P){
  data.frame(
    f_s1 = u[,1] - u[,3] + beta * log(P[1,3]) - beta * log(P[,3]),
    f_s2 = u[,2] - u[,4] + beta * log(P[1,4]) - beta * log(P[,4])
  )
}
